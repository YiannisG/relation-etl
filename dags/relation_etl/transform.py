...
def _merge_duplicates(records: list[dict], key: str, table: str, result: TransformResult) -> dict[str, dict]:
    merged: dict[str, dict] = {}
    for rec in records:
        pk = rec.get(key)
        if pk is None:
            result.quarantine.append(
                QuarantineRecord(table=table, record=rec, reason=f"missing primary key '{key}'")
            )
            continue
        if pk not in merged:
            merged[pk] = dict(rec)
...
def _valid_exon_coords(start, end) -> tuple[bool, str | None]:
    ...
    if end < start:
        return False, f"end is <= start"
    if (end - start) > MAX_EXON_LENGTH:
        return False, f"length exceeds specified maximum length"
    return True, None
...
def transform(raw: dict[str, list[dict]]) -> TransformResult:
    ...
    # genes
    merged_genes = _merge_duplicates(records=raw["genes"], key="gene_id", table="genes", result=result)
    ...
    # transcripts
    merged_transcripts = _merge_duplicates(records=raw["transcripts"], key="transcript_id", table="transcripts", result=result)
    clean_transcripts = {}
    for tx_id, tx in merged_transcripts.items():
        if tx.get("gene_id") not in gene_ids:
            result.quarantine.append(QuarantineRecord(
                table="transcripts", record=tx, reason=f"orphan: gene_id {tx.get('gene_id')!r} not found in genes"
            ))
            continue
    ...
    # exons
    merged_exons = _merge_duplicates(records=raw["exons"], key="exon_id", table="exons", result=result)
    clean_exons = []
